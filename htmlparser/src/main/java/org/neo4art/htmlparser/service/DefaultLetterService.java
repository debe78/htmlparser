...
public class DefaultLetterService implements LetterService{
	...
	private Letter elaboraFile(File file) throws LetterParserException {
		...
		try {
			...
			//Inventory
			letter.setInventory(parseLetterContent(listContentFile, 6));
			//Prendo il numero di link dell'immagine di una lettera
			int numero = getNumberLinkImage(listContentFile, 7);
			//Setto i link delle immagini delle lettera dentro un Vector
			letter.setLink(parseLetterLinkImageContent(listContentFile, 7 + numero));				
			//Url
			letter.setUrl(parseLetterContent(listContentFile, 7 + numero));
			//Letter
            letter.setText(parseLetterTextContent(listContentFile,8 + numero));
	 ...
	 * @return
	 */
	private String parseLetterTextContent(List<String> listContentFile, int indice) throws LetterParserException{
		...
		if(listContentFile.get(indice).startsWith("Letter|")){
			...
			throw new LetterParserException("A letter not start with Letter|");
		}
